...
const LABELS = {
  ...
  'low-stock': 'Low Stock', 'movement-register': 'Movement Register',
};
...
const SellerAnalytics = () => {
  ...
  useEffect(() => {
    getSellerReportList().then((r) => setReports(listOf(r))).catch((e) => { if (isUpgrade(e)) setLocked(true); });
  ...
  const fetchRows = () => {
    ...
    return runSellerReport(name, params)
  ...
  }, [name]); // re-run when the report changes

  const columns = useMemo(() => (rows[0] ? Object.keys(rows[0]) : []), [rows]);
  const download = async () => {
    try { await downloadSellerReportCsv(name, Object.fromEntries(Object.entries(filters).filter(([, v]) => v))); } catch (e) { apiError(e); }
  };
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <select value={name} onChange={(e) => setName(e.target.value)} className="block border border-stone-200 rounded-lg text-sm px-3 py-2 bg-white font-medium mt-1">
              {reports.map((r) => <option key={r.name} value={r.name}>{LABELS[r.name] || r.name}</option>)}
            </select>
          </div>
          <div>
